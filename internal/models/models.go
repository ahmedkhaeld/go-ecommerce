...
// GetUserByEmail return potentially a user if it finds one, and a potentially an error
// allows looking up a user by email address
func (m *DBModel) GetUserByEmail(email string) (User, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	// forcefully store any email by lowercase, so use strings.ToLower()
...
// GetAllOrders list all orders data
func (m *DBModel) GetAllOrders() ([]*Order, error) {
	...
	query := `
		select 
		   ...
		   w.id, w.name,
		   t.id, t.amount, t.currency, t.last_four, t.expiry_month, t.expiry_year, t.payment_intent,
		   t.bank_return_code,
...
func (m *DBModel) GetAllOrdersPaginated(pageSize, page int) ([]*Order, int, int, error) {
	...
	// count the # of orders for widgets as is_recurring = 0
	query = `
		select count(o.id)
		from orders o 
		left join widgets w on (o.widget_id = w.id)
